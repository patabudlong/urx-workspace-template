...
import { EMAIL_ASSET_PREFIX } from '../src/lib/shared/mail/email-asset-names.ts';
import { ensurePublicEmailAssetsPolicy } from '../src/lib/server/storage/linode-bucket-policy.ts';
...
async function main() {
	...
	const assetsDir = join(ROOT_DIR, 'static', 'email');
	...
	console.log(`Ensuring public read policy for ${config.bucket}/${EMAIL_ASSET_PREFIX}/*`);
	await ensurePublicEmailAssetsPolicy(client, config, EMAIL_ASSET_PREFIX);

	console.log(`Uploading ${files.length} email assets to ${config.publicBase}/${EMAIL_ASSET_PREFIX}/`);
	...
	for (const file of files) {
		...
		const key = `${EMAIL_ASSET_PREFIX}/${file}`;
