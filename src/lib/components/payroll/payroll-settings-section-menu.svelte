<script lang="ts">
	...
	import { PAYROLL_SETTINGS_NAV_ITEMS } from '$lib/navigation/payroll-settings-nav';
	...
	const isSettingsActive = $derived(page.url.pathname.startsWith('/payroll/settings'));
...
<div>
	...
	<div class="px-1">
		...
		>
			<AppIcon icon={PAYROLL_SETTINGS_NAV_ITEM.icon} class="shrink-0" aria-hidden="true" />
		...
		{#if settingsOpen}
			<div class="border-sidebar-border mt-1 ms-5 flex flex-col gap-1 border-l ps-2.5">
				{#each PAYROLL_SETTINGS_NAV_ITEMS as item (item.href)}
