import { canAccessDtrWorkspace, canManageDtr } from '$lib/shared/dtr/access';
...
export async function requireDtrMemberWorkspace(input: {
	userId: string | undefined;
	url: URL;
	requestId?: string;
}): Promise<DtrApiContextResult> {
	return requireWorkspaceModuleApiContext({
		...input,
		packageId: WORKSPACE_PACKAGE_IDS.DTR,
		requireRole: canAccessDtrWorkspace,
