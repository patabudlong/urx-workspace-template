...
{:then mailbox}
	<Card.Root class="flex min-h-0 min-w-0 flex-1 flex-col gap-0 py-0">
		{@const hasPagination = mailbox.pagination.page > 1 || mailbox.pagination.hasMore}
		{#if !isInboxFolder || hasPagination}
			<Card.Header
				class="border-border shrink-0 flex-row items-center justify-between gap-4 space-y-0 border-b px-4 pt-4 pb-4"
			>
				{#if !isInboxFolder}
					<div>
