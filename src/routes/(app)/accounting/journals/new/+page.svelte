<script lang="ts">
	...
	import { ACCOUNTING_JOURNAL_SAVE_FAILED_MESSAGE } from '$lib/shared/accounting/messages';
...
<div class="flex w-full min-w-0 flex-col gap-8">
	<PageHeader
		...
		description="Post a balanced manual journal to an open fiscal period."
	/>
	...
	{#if typeof $formMessage === 'string' && $formMessage.length > 0}
		<StatusAlert
			...
			description={$formMessage === ACCOUNTING_JOURNAL_SAVE_FAILED_MESSAGE
	...
	<form method="POST" use:enhance class="space-y-6">
		<Card.Root>
			...
			<Card.Content class="grid gap-4 md:grid-cols-2">
				<div class="space-y-2">
					<Label for="periodId">Fiscal period</Label>
					<select
						id="periodId"
						name="periodId"
						class="border-input bg-background h-10 w-full rounded-md border px-3 text-sm"
						bind:value={$form.periodId}
						required
					>
						{#each data.periods as period (period.id)}
							<option value={period.id} disabled={period.status !== 'open'}>
		...
		<Card.Root>
			<Card.Header class="flex flex-row items-center justify-between gap-4">
				<div>
					...
					<Card.Description>Each line must have either a debit or a credit amount.</Card.Description>
				</div>
