<script lang="ts">
	...
	const loginHref = $derived(`/login?redirectTo=${encodeURIComponent(acceptPath)}`);
	const signupHref = $derived(`/signup?redirectTo=${encodeURIComponent(acceptPath)}`);
</script>
...
>
	...
	{:else if data.emailMatches === false}
		...
		/>
		<Button href="/logout" class={AUTH_ACTION_BUTTON_CLASS}>Sign out</Button>
	{:else}
