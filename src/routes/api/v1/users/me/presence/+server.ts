...
import { findUserById, updateUserPresenceStatus } from '$lib/server/repositories/users';
import { updatePresenceStatusSchema } from '$lib/shared/schemas/presence';
...
export const PATCH: RequestHandler = async ({ locals, request }) => {
	...
	const parsed = updatePresenceStatusSchema.safeParse(body);
	...
	const updated = await updateUserPresenceStatus(locals.user.id, parsed.data.status);
