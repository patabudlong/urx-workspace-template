...
import { createCrmDeal, listCrmDeals } from '$lib/server/repositories/crm-deals';
import { requireCrmWorkspace } from '$lib/server/crm/api-context';
import { createCrmDealSchema, crmListQuerySchema } from '$lib/shared/crm/schemas';
...
export const GET: RequestHandler = async ({ locals, request, url }) => {
	...
	const { items, total } = await listCrmDeals({
...
export const POST: RequestHandler = async ({ locals, request, url }) => {
	...
	if (!parsed.success) {
		return jsonError('BAD_REQUEST', 'Invalid deal payload', {
			details: { issues: parsed.error.flatten() },
			requestId
		});
	}
	...
	return jsonOk(deal, { requestId, status: 201 });
};
